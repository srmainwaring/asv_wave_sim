...
bool WavesClient::Impl::InitWavefield(EntityComponentManager &ecm)
{
  ...
  std::string entity_name = "wavefield";
  wavefield_entity_ = ecm.EntityByComponents(components::Name(entity_name));
    ...
    EntityComponentManager &/*ecm*/)
{
  ...
  wavefield_.lock()->Height(point, wave_height);
  gzmsg << "[" << sim_time << "] : " << wave_height << "\n";

  // cannot modify the wavefield
  // wavefield_.lock().Update(sim_time);
}
...
GZ_ADD_PLUGIN(
    ...
    gz::sim::System,
    gz::sim::systems::WavesClient::ISystemConfigure,
    gz::sim::systems::WavesClient::ISystemPreUpdate)
